...
**X-ASR (STT) → Granite-4.2-3B (LLM + native tools) → Qwen3-TTS 0.6B Q8 (streaming TTS)**
...
- **Real models only** (`mock=false` everywhere):
  ...
  - LLM: `ibm-granite/granite-4.2-3b-GGUF` Q4_K_M — llama-server, native `tools=[web_search, get_current_datetime]` (agent-strong; reads tool results reliably incl. EN).
...
- LLM: `/tmp/llms/granite-4.2-3b-Q4_K_M.gguf` (or `--llm` override)
...
Granite-4.2-3B (Apache-2.0), Gemma-4-E2B-it (Gemma license), Qwen3.5-4B-MTP (Apache-2.0), Qwen3-TTS (Apache-2.0), X-ASR (Apache-2.0),
