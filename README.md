...
- **zh-TW by default, English mixed in** — the harness system messages unconditionally default every reply to Traditional Chinese (Taiwan usage), regardless of the input's language — English is kept only for untranslatable terms/proper nouns. The instruction is stated **once, in the system prompt**, and deliberately never appended to the user's own turn: glued onto the transcript it became part of what the model was asked, so it was copied verbatim into `web_search` queries (literal parenthetical searches returning `no_results`) and read back aloud as the answer. TTS segments text by script and passes the correct explicit language per segment (`chinese`/`english`) instead of a single whole-utterance `"auto"` guess, so a name or term quoted mid-sentence doesn't get the wrong pronunciation. Default voice, UI text, and quick-demo chips are all zh-TW; the frontend's display-layer OpenCC conversion uses the phrase-aware `s2twp` preset (Taiwan vocabulary, not just Traditional glyphs) as a display-only safety net.
...
A UI-level sweep — the four quick-chip prompts against all three model sizes, over the
real WebSocket path — is what surfaced the tool-routing gaps above. Two lessons from it
are worth keeping if you write your own:

- **Assert on what the UI renders.** The first version accumulated the `token` field
  while the frontend renders `text_so_far`, and ignored `llm_reset`. It reported 12/12
  green against an answer string no user would ever see. A client under test should
  mirror `handleServerMessage` rather than reconstruct its own idea of the transcript.
- **Check the answer bubble, not just the audio.** Reasoning kept out of speech can
  still be sitting in the transcript; those are two separate assertions.
...
- **Thinking shown, never spoken** — `Qwen-Agent` `enable_thinking:True`. `reasoning_content` is surfaced as its own `reasoning` event and rendered in a collapsible panel, but never reaches TTS or chat history. When llama-server's `reasoning_budget_tokens` cap truncates the thinking block, the remainder spills into `content` as plain English deliberation, so every path that can reach TTS re-checks and re-routes it. A single token is too small a unit to classify, so a sentence recognized as reasoning only after it has streamed is also **retracted from the answer bubble** (`app.py:_retract`) — otherwise speech stays clean while the transcript shows thinking as the answer. Detection is deliberately not a list of observed phrases: instruction echoes are matched by character-shingle overlap against our own prompt text (so truncated/paraphrased replays are caught), and a standalone sentence-length all-English span is treated as a non-answer, since answers here are zh-TW with English only *inside* a Chinese sentence.
- **Tool routing by question form, not model whim** — the current date and a current office holder are no more present in the weights than tomorrow's weather is, so questions whose *form* demands a lookup are routed to the tool before the model answers (`agent/qwen_harness.py:required_tool_for_request`). The Chinese patterns match the ordinary word order (`總統是誰`, 誰 after the office noun) as well as the verb-first `誰是` — matching only the latter meant the app's own `法國現在的總統是誰？` demo chip was answered from memory. Lookup intent is tested before the clock so `現在` in that question doesn't turn it into a time query, and weather keeps `今天` in `今天天氣`. `_preflight_enabled()` turns these off to measure the bare model.
