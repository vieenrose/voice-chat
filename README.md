...
**X-ASR (STT) → Granite-4.2-8B (LLM + native tools) → Qwen3-TTS 0.6B Q8 (streaming TTS)**
...
- **Real models only** (`mock=false` everywhere):
  ...
  - LLM: `ibm-granite/granite-4.2-8b-GGUF` Q4_K_S — llama-server, native `tools=[web_search, get_current_datetime]` (8B agent-strong, reliable tool-result reading, incl. EN weather).
...
- LLM: `/tmp/llms/granite-4.2-8b-Q4_K_S.gguf` (or `--llm` override)
...
Granite-4.2-8B (Apache-2.0), Gemma-4-E2B-it (Gemma license), Qwen3.5-4B-MTP (Apache-2.0), Qwen3-TTS (Apache-2.0), X-ASR (Apache-2.0),
