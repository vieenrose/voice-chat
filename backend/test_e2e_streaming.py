...
async def main(mock=True, iters=10):
    ...
    for i, txt in enumerate(texts):
        ...
        if not mock:
            ...
            if not stt_text.strip():
                ...
                stt_text=txt  # keep deterministic
        ...
        async for ev in pipe.llm.generate_stream(stt_text):
            if ev["type"]=="llm_token":
                ...
                if SENT_END.search(ev["token"]):
                    flush=True
                elif cnt>=8 and tts_buf and tts_buf[-1] in " ,":
                    flush=True
        ...
        print(f" iter {i+1:02d} stt={stt_ms:3d} llm_ttft={llm_ttft:3d} tts_ttfb={tts_ttfb:3d} e2e={e2e:3d} (streaming overlap) — '{txt[:35]}'")
    ...
    if e2e_list:
        ...
        if avg<800:
            print(f"  ✅ PASSED <800ms (avg {avg:.0f}ms) — streaming overlap working")
        else:
            print(f"  ❌ avg {avg:.0f}ms >800ms")
    print("="*78+"\n")
